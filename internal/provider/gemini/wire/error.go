...
const (
	errorPrefix      = "gemini provider: "
	maxErrorBodySize = 64 * 1024
)

// ParseError 解析 Gemini HTTP 错误响应，并限制错误体最大读取长度。
func ParseError(resp *http.Response) error {
	...
	if err := json.Unmarshal(data, &parsed); err == nil && strings.TrimSpace(parsed.Error.Message) != "" {
		return provider.NewProviderErrorFromStatus(resp.StatusCode, strings.TrimSpace(parsed.Error.Message))
	}
